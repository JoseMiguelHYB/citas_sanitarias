...
public class CitaService {
	...
	public Cita guardar(Cita cita) {
		// Validar: no puede haber otra cita igual (mismo paciente, médico, fecha y
		// hora)
		boolean existeConflicto;

		if (cita.getId() == null) {
			// Creación → basta con comprobar si existe
			existeConflicto = citaRepository.existsByPacienteIdAndMedicoIdAndFechaAndHora(cita.getPaciente().getId(),
			...
			existeConflicto = citaRepository.existsByPacienteIdAndMedicoIdAndFechaAndHoraAndIdNot(
					cita.getPaciente().getId(), cita.getMedico().getId(), cita.getFecha(), cita.getHora(),
					cita.getId());
		}
